...
@dataclasses.dataclass
class ConfigLingmoOSarchDefs:
    __arch: Optional[str] = dataclasses.field(default=None, init=False)

    def __post_init_defs__(self, parent: ConfigLingmoOSarch) -> None:
...
class ConfigKernelarch(ConfigBase):
    debianarch: list[ConfigLingmoOSarch] = dataclasses.field(
        default_factory=list, metadata={'merge': 'assoclist'},
    )
...
@dataclasses.dataclass
class ConfigLingmoOSarch(ConfigBase):
    defs: ConfigLingmoOSarchDefs = dataclasses.field(default_factory=ConfigLingmoOSarchDefs)
...
class ConfigFeatureset(ConfigBase):
    ...
    def __post_init_hierarchy_featureset__(
        self,
        path: Path,
        debianarch: Optional[ConfigLingmoOSarch],
    ) -> None:
...
class ConfigMergedKernelarch(ConfigMerged):
    ...
    @property
    def debianarchs(self) -> Iterable[ConfigMergedLingmoOSarch]:
        for debianarch in self._kernelarch.debianarch:
            yield ConfigMergedLingmoOSarch(
                root=self._root,
...
class ConfigMergedLingmoOSarch(ConfigMergedKernelarch):
    _debianarch: ConfigLingmoOSarch
    ...
    def __init__(
        ...
        **kw: Optional[ConfigBase],
        ...
        if debianarch is not None:
            assert isinstance(debianarch, ConfigLingmoOSarch)
    ...
    @property
    def defs_debianarch(self) -> ConfigLingmoOSarchDefs:
...
class ConfigMergedFeatureset(ConfigMergedLingmoOSarch):
...
#    pprint(config)
    ...
    for kernelarch in merged.kernelarchs:
        ...
        for debianarch in kernelarch.debianarchs:
            print_indent(
                1,
                f'LingmoOSarch: {debianarch.name}',
